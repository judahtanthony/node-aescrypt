...
class CLI {
  ...
  public async execute(): Promise<any> {
    let { decrypt, input, output, password } = this.options;
    let tmpFile = '';
    if (!password) {
      ...
      if (input[0] === '-') {
        ...
        unlinkSync(tmpFile);
        this.error('Error: You must provide the password if you are stream the data on STDIN.');
      }
      password = await this._getPasswordOnStdIn();
    }
    // This would be a great use of async.
    return Promise.all(
      input.map((infile: string) => new Promise((resolve, reject) => {
  ...
  private _getToStream(input: string, output?: string, decrypt?: boolean): Writable {
  ...
  private _stashStdIn(): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      const outfile = pathJoin(tmpdir(), 'tmp' + randomBytes(16).toString('base64'));
